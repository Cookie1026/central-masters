...
csv_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\第78回PDF元データ(anti)V8.csv"
...
def split_merged_item(item):
    ...
    # 1. まず順位が先頭に密着している場合を分離 (例: 1,森川スェ子...)
    m_rank = re.match(r'^(\d+),', item)
    rank = ""
    if m_rank:
        rank = m_rank.group(1)
        item = item[m_rank.end():]
    ...
    if m_lane:
        ...
        # 3. 残りの末尾から「年齢 (例: 117歳)」または「所属 (セ・...)」を探す
        m_age = re.search(r'(\d+歳)$', rest)
        if m_age:
            age = m_age.group(1)
            name = rest[:m_age.start()]
            res = [name, age, lane]
            return ([rank] + res) if rank else res
    ...
    # 4. 水路がない場合でも「所属」が密着しているケース (例: 栃元春乃セ・西東京)
    ...
    return [rank + item] if rank else [item]
...
try:
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages):
            ...
            text = pdfplumber.utils.extract_text(modified_chars, x_tolerance=2.5, y_tolerance=3)
            ...
            text = text.replace('大会己へ', '大会記録').replace('世界己へ', '世界記録').replace('日本己へ', '日本記録')
    ...
    print(f"V8 successfully generated: {csv_path}")
