...
csv_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\第78回PDF元データ(anti)V16.csv"
mapping_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\pua_mapping.json"
memo_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\メモ(anti).txt"
...
NAME_REPLACEMENTS = {
    ...
    "長野愛子子": "長野愛子", "ザス川崎": "ザバス川崎", "ザバス川崎崎": "ザバス川崎"
}

memos = []

def clean_all(s, line_no=None, original_row=None):
    orig = s
    ...
    if line_no and (flagged or '毛海岸' in s or 'ィ' in orig or '0' in orig):
        memos.append(f"行{line_no}: 修正・文字欠け疑い 「{orig} -> {s}」 行: '{' '.join(original_row)}'")
    
    return s.strip()

def split_merged_item(item):
    # A. 氏名 + (セ・|ザバス|ザス) + 水路
    m = re.search(r'^(.*?)(セ.+?|ザバス.+?|ザス.+?)(\d+/\d*)$', item)
...
def process_row(row, line_no):
    original_row_str = [str(x) for x in row]
    # 1. ヘッダ統合 (時 + 間)
    i = 0
    while i < len(row) - 1:
    ...
    for idx in range(1, len(row)):
        if any(k in row[idx] for k in ['セ', '歳', 'ザバス', 'ザス']):
    ...
    return [clean_all(item, line_no, original_row_str).replace('宀', '宮').replace('っ', '5') for item in row if item]
...
try:
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages):
            ...
            for line in text.split('\n'):
                ...
                processed = process_row(row, current_line_no)
    ...
    print(f"V16 generated with memo: {csv_path}")
