...
csv_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\第78回PDF元データ(anti)V10.csv"
...
def split_merged_item(item):
    ...
    if '歳' in item and '/' in item:
        ...
        m_lane = re.search(r'\d+/\d*$', item)
        if m_age and m_lane:
            team = item[:m_age.start()]
            age = m_age.group()
            lane = m_lane.group()
            return [team, age, lane]
...
try:
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_idx, page in enumerate(pdf.pages):
            ...
            text = pdfplumber.utils.extract_text(modified_chars, x_tolerance=1.5, y_tolerance=3)
    ...
    print(f"V10 successfully generated: {csv_path}")
