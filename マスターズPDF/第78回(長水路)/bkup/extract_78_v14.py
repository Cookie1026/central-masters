...
csv_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\第78回PDF元データ(anti)V14.csv"
...
def process_row(row):
    ...
    # 3. リレーメンバー姓名結合 (判定の厳格化)
    has_lane = any(re.match(r'^\d+/\d*$', item) for item in row)
    has_club = any('セ' in item for item in row)
    has_age  = any('歳' in item for item in row)

    # 「水路」「所属」「合計年齢」のどれも持たず、順位1-4から始まる行だけをリレーメンバー行とする
    if len(row) >= 3 and row[0] in ['1', '2', '3', '4'] and not (has_lane or has_club or has_age):
        time_idx = -1
...
try:
    ...
    print(f"V14 successfully generated: {csv_path}")
