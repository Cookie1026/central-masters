...
csv_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\第78回PDF元データ(anti)V7.csv"
...
def split_merged_item(item):
    # 1. 順位(数字,) + 氏名 + セ・所属 + 水路
    m = re.match(r'^(\d+,?)(.+?)(セ・.+?)(\d+/\d*)$', item)
    if m: return [m.group(1), m.group(2), m.group(3), m.group(4)]
    
    # 2. 氏名 + セ・所属 + 水路
    m = re.match(r'^(.+?)(セ・.+?)(\d+/\d*)$', item)
    if m: return [m.group(1), m.group(2), m.group(3)]
    
    # 3. チーム名 + 歳 + 水路
    m = re.match(r'^(.+?)(\d+歳)(\d+/\d*)$', item)
    if m: return [m.group(1), m.group(2), m.group(3)]
...
try:
    ...
    print(f"V7 successfully generated: {csv_path}")
