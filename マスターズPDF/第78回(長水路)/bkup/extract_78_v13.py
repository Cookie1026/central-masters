...
csv_path = r"c:\Users\user\OneDrive\デスクトップ\antigravity\central_masters\元PDF\第78回(長水路)\第78回PDF元データ(anti)V13.csv"
...
def process_row(row):
    ...
    # 3. リレーメンバー姓名結合 (順位1-4の行のみ)
    # リレーメンバー行の判定: 1列目が数字かつ2列目が「セ・」を含まず「歳」も含まない
    if len(row) >= 3 and row[0] in ['1', '2', '3', '4'] and 'セ' not in row[1] and '歳' not in row[1]:
        # タイムや記号が出るまでを姓名として結合
        time_idx = -1
...
try:
    ...
    print(f"V13 successfully generated: {csv_path}")
